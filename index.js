...
async function run() {
  try {
    ...
    const verifyAdmin = async (req, res, next) => {
      ...
      if (user?.role !== 'admin') {
        return res.status(403).send({ error: true, messages: 'forbidden messages' })
      }
      next()
    }

    /**
     * 0. do not show secure link for any user
     * 1. useJWT token : verifyJwt
     * 2. use verify admin middle
     * * */
    ...
    app.get('/carts', verifyJwt, async (req, res) => {
      ...
      if (email !== decodedEmail) {
        return res.status(403).send({ error: true, messages: 'forbidden access' })
      }
